"""Remove mis-ordered appendix and re-insert in correct order."""
...
BLOCKS = [
    ...
    ("list", "Publication impact: Medium-High - turns the project into a defense-comparison study."),
    ...
    ("list", "attacks/pgd_attack.py - PGD-Linf generator (k=10 steps, step size alpha = epsilon/4)."),
    ...
    (
        "list",
        "attacks/fgsm_attack.py - import generate_fgsm() from attack_utils; remove duplicated "
        "backward logic.",
    ),
    (
        "list",
    ...
    (
        ...
        "Robust CNN across epsilon in {0.01, 0.03, 0.05, 0.10}.",
    ),
    ("sub", "B.3 Additional Tables and Figures (after experiments complete)"),
    (
        "list",
        "TABLE VI - Baseline CNN accuracy under FGSM and PGD (four epsilon rows, two attack columns).",
    ),
    (
        "list",
    ...
    (
        "list",
        "TABLE VIII - Robustness gap: (Robust PGD accuracy - Baseline PGD accuracy) per epsilon.",
    ),
    ...
    ("list", "Title - add evaluation scope, e.g., FGSM and PGD Attacks."),
    (
        "list",
        "Abstract - keep all current FGSM results; add one sentence on PGD evaluation after runs complete.",
    ),
    (
        "list",
        "Methodology - add subsection Projected Gradient Descent (PGD) Attack with k=10, alpha=epsilon/4.",
    ),
    ("list", "Results - add subsection Cross-Attack Robustness with Tables VI-VIII and Figure 8."),
    ("list", "Discussion - interpret FGSM-PGD gap and threat-model mismatch honestly."),
    ("list", "Conclusion - broaden claim to evaluated threat model; retain all FGSM conclusions."),
    ...
    (
        "list",
        "Step 3 - Run evaluation/multi_attack_eval.py on both checkpoints; save results/attack_benchmark.csv.",
    ),
    ...
    ("list", "Step 6 - Add Madry PGD citation; keep all five existing references."),
    (
        "body",
...
def insert_before(doc: Document, anchor: Paragraph, text: str, kind: str) -> None:
...
def remove_appendix(doc: Document) -> None:
    refs = find_paragraph(doc, "9. References")
    if refs is None:
        raise SystemExit("References not found")
    marker = find_paragraph(
        doc,
        "Each extension would reuse the same logging format so new runs stay comparable to the numbers reported here.",
    )
    if marker is None:
        raise SystemExit("Future work end marker not found")

    to_remove = []
    capture = False
    for p in doc.paragraphs:
        if p._element is marker._element:
            capture = True
            continue
        if p._element is refs._element:
            break
        if capture:
            to_remove.append(p)
...
def main() -> None:
    ...
    for kind, text in BLOCKS:
        insert_before(doc, refs, text, kind)
    doc.save(SRC)
    print(f"Fixed appendix order in {SRC}")
